...
PROJECTNAME := "fury-kubernetes-tracing"
...
## deploy-all: Deploys all the components in the tracing module
deploy-all: deploy-minio deploy-tempo

## deploy-minio: Deploys the `minio` component in the cluster
deploy-minio: check-kustomize check-kubectl
	@kustomize build katalog/minio-ha | kubectl apply -f-

## deploy-tempo: Deploys the `tempo` component in the cluster
deploy-tempo: check-kustomize check-kubectl
	@kustomize build katalog/tempo-distributed | kubectl apply -f-
